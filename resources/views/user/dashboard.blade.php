    ...
    <div class="py-8 space-y-8">
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-3xl md:text-4xl font-black text-white tracking-tight mb-3 flex items-center gap-3">
                        {{ $greeting }}, {{ Auth::user()?->name ?? 'User' }} <span class="animate-bounce" style="animation-duration: 2s; transform-origin: bottom center;">👋</span>
                    </h1>
        ...
        <div class="flex flex-col space-y-8 pb-8">
                        ...
                        <table class="w-full text-left border-collapse">
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr onclick="window.location='{{ route('tickets.show', $ticket->id) }}'" class="hover:bg-blue-50/50 transition duration-200 group/row cursor-pointer relative">
                                    ...
                                    <td class="px-6 py-5 text-right relative z-10 w-48 whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-4">
                                            @php
                                                ...
                                                $badgeClass = $statusColors[$ticket?->status ?? 'Open'] ?? 'bg-gray-100 text-gray-700 border-gray-200';
                                            @endphp
                                ...
                                @empty
                                <tr>
                                    <td colspan="2" class="px-6 py-16 text-center whitespace-nowrap">
                                ...
                                <tr class="hover:bg-orange-50/30 transition duration-150 group">
                                    ...
                                    <td class="px-6 py-5 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-2">
                                            @if($asset?->isOnLoan())
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 shadow-sm mr-2">
                                                    <x-lucide-arrow-right-left class="w-3.5 h-3.5 mr-1.5" />
                                                    Dipinjamkan ke {{ $asset?->activeLoan?->borrower?->name ?? 'Seseorang' }}
                                                </span>
                                            @else
                                                <button type="button" onclick="openQrModal('{{ route('assets.scan', $asset?->id ?? 0) }}?mode=loan', '{{ $asset?->deviceName?->name ?? 'Device' }}', 'loan')" class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors shadow-sm" title="Pinjamkan Sementara">
                                                    <x-lucide-refresh-cw class="w-4 h-4 mr-1.5" />
                                                    Pinjamkan
                                                </button>
                                            @endif
                                            ...
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-bold bg-white text-gray-700 border border-gray-200 ml-2">
            ...
            <div class="flex flex-col space-y-4">
                <div class="flex justify-between items-end px-2">
                    <div>
                        <h3 class="text-xl font-bold text-gray-900">Aset yang Saya Pinjam</h3>
                        ...
                        <table class="w-full text-left border-collapse">
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-blue-50/30 transition duration-150 group">
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="flex items-start gap-4">
                                            <div class="p-2.5 rounded-xl bg-gray-50 text-gray-400 group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors">
                                                <x-dynamic-component :component="'lucide-' . ($asset->deviceName?->type == 'Laptop' ? 'laptop' : 'monitor')" class="w-5 h-5" />
                                            </div>
                                            <div>
                                                ...
                                                <div class="text-xs font-medium mt-1.5 text-blue-600">
                                                    Pemilik: {{ $asset?->activeLoan?->lender?->name ?? 'BPS' }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-3">
                                            <form action="{{ route('assets.return', $asset->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" onclick="return confirm('Apakah Anda yakin ingin mengembalikan perangkat ini?')" class="inline-flex items-center px-4 py-2 rounded-xl text-xs font-bold bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 hover:text-red-600 transition-colors shadow-sm">
                                                    <x-lucide-corner-up-left class="w-4 h-4 mr-1.5" />
                                                    Kembalikan
                                                </button>
                                            </form>
                                        </div>
                                ...
                                <tr>
                                    <td colspan="2" class="px-6 py-16 text-center whitespace-nowrap">
                                        <div class="flex flex-col items-center max-w-[280px] mx-auto">
                                            <div class="w-16 h-16 bg-gray-50 rounded-2xl flex items-center justify-center mb-4 border border-dashed border-gray-200">
                                                <x-lucide-check-circle class="w-8 h-8 text-gray-300" />
                                            </div>
                                            <h4 class="text-gray-900 font-bold mb-1">Tidak Ada Pinjaman</h4>
                                            <p class="text-sm text-gray-500 leading-relaxed">Anda tidak sedang meminjam perangkat apapun saat ini.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
            ...
            <div class="relative bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:max-w-md sm:w-full border border-gray-100 z-10">
                <div class="px-6 py-6 text-center" id="qrHeaderBg">
                    <h3 class="text-xl leading-6 font-bold text-white flex items-center justify-center gap-2" id="modal-title">
                        <x-lucide-smartphone class="w-6 h-6" /> <span id="qrTitleText">Scan</span>
                    </h3>
                    <p class="text-blue-100 text-sm mt-2" id="qrDescriptionText">Minta rekan Anda memindai kode QR ini menggunakan HP mereka untuk memproses <span id="qrDeviceName" class="font-semibold text-white"></span>.</p>
                </div>
                <div class="px-6 py-8 flex flex-col items-center bg-gray-50 border-t border-b border-gray-100">
                    <div id="modalQrRender" class="p-4 bg-white rounded-2xl shadow-md border border-gray-100 transition-transform transform hover:scale-105 duration-300"></div>
                    <p class="text-xs text-gray-400 mt-6 max-w-xs text-center font-medium leading-relaxed">
                        Aksi akan tercatat secara otomatis saat rekan Anda mengkonfirmasi persetujuan di HP mereka.
                    </p>
                </div>
                <div class="bg-white px-6 py-4 flex flex-row-reverse border-t border-gray-100">
                    <button type="button" onclick="closeQrModal()" class="w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-6 py-3 bg-white text-base font-bold text-gray-700 hover:bg-gray-50 hover:text-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors sm:w-auto sm:text-sm">
                        Tutup
                    </button>
                </div>
            </div>
