<x-app-layout>
    <div class="space-y-6">
        <div class="sm:flex sm:items-center sm:justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                    <x-lucide-edit class="w-6 h-6 text-bps-orange" />
                    Edit Artikel FAQ
                </h1>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <form action="{{ route('admin.faq.articles.update', $faq->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <div>
                        <label for="question" class="block text-sm font-medium text-gray-700">Pertanyaan / Topik <span class="text-red-500">*</span></label>
                        <input type="text" name="question" id="question" value="{{ old('question', $faq->question) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-bps-blue focus:border-bps-blue sm:text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="is_published" id="is_published" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-bps-blue focus:border-bps-blue sm:text-sm">
                                <option value="1" {{ old('is_published', $faq->is_published) == '1' ? 'selected' : '' }}>Publikasikan (Live)</option>
                                <option value="0" {{ old('is_published', $faq->is_published) == '0' ? 'selected' : '' }}>Simpan sebagai Draft</option>
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="answer" id="answer" rows="10" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-bps-blue focus:border-bps-blue sm:text-sm">{!! old('answer', $faq->answer) !!}</textarea>
                ...
                <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-100">
                    ...
                    <button type="submit" class="inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-bps-blue text-base font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-bps-blue sm:text-sm transition-colors">
